...
| **Governance Gate** | The decision point in the ANIF pipeline, evaluated by the Policy Engine and Decision Engine, that classifies an intent as `auto`, `manual_review`, `block`, or (in L5 deployments) `council_review`. The governance gate is mandatory and MUST NOT be bypassed. | Stage 4 of the pipeline | Governance Mode, Policy Evaluation, Human Override (P-06) |
| **Governance Mode** | One of four possible classifications produced by the Governance Gate: `auto` (action may proceed without human review), `manual_review` (action is queued for human approval before proceeding), `block` (action is permanently rejected), or `council_review` (action is referred to the AI Council for deliberation — L5 deployments only, as defined in ANIF-900). | Decision output | Governance Gate, Governance Ticket, Human Override |
